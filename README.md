...
Helps to implement swipeable pages. `PageSlider` is controlled with `selectedPage` prop component and
...
```sh
yarn add arunchaitanyajami/react-native-page-slider
```

or using npm

```sh
npm install -S arunchaitanyajami/react-native-page-slider
```
...
import { PageSlider } from 'arunchaitanyajami/react-native-page-slider';
